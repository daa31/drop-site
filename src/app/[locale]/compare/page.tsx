...
export default function ComparePage() {
  ...
  if (!items.length) return <div className="container-f py-20">Немає товарів для порівняння.</div>;
  return (
    <div className="container-f overflow-x-auto py-10">
      <h1 className="font-display text-3xl">Порівняння</h1>
      <table className="mt-8 min-w-[640px] w-full text-sm">
        ...
        <tbody>
          <tr className="border-t">
            <td className="p-3 text-graphite/50">Ціна</td>
            {items.map((i) => (
              <td key={i.slug} className="p-3">{i.retailPrice}</td>
            ))}
          </tr>
          <tr className="border-t">
            <td className="p-3 text-graphite/50">Бренд</td>
            {items.map((i) => (
